...
import { DEFAULT_MAX_CONTRACTS, DEFAULT_PLAN, shouldSkipQuota } from './usersRepo.js';
export async function createAnalysisJob(uid, input, token) {
...
export async function markJobProcessing(jobId, analysisId) {
    const firestore = getFirestore();
    const now = new Date().toISOString();
...
export async function completeAnalysisJob(input) {
    ...
    await firestore.runTransaction(async (transaction) => {
        ...
        const sanitized = Object.fromEntries(Object.entries(input.analysisFields).filter(([key]) => key !== forbiddenRawTextField));
...
export async function failAnalysisJob(input) {
    ...
    await firestore.runTransaction(async (transaction) => {
        ...
        if (userSnapshot.exists) {
            const inFlight = typeof userSnapshot.data()?.contractsInFlight === 'number'
