...
export async function deleteAnalysisForUser(uid, analysisId) {
    ...
    const contractsAnalyzed = await firestore.runTransaction(async (transaction) => {
        ...
        const current = typeof userData.contractsAnalyzed === 'number' ? userData.contractsAnalyzed : 0;
        const inFlight = typeof userData.contractsInFlight === 'number' ? userData.contractsInFlight : 0;
