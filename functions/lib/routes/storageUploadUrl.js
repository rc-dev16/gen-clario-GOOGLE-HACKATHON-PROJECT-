...
import { ALLOWED_UPLOAD_MIME_TYPES, MAX_UPLOAD_BYTES, SIGNED_URL_TTL_MS, requireUploadBucket } from '../config.js';
...
export async function handleUploadUrl(req, user) {
    ...
    try {
        ...
        throw new HttpError(500, 'SIGNED_URL_FAILED', 'Could not create upload URL. For local dev, set GOOGLE_APPLICATION_CREDENTIALS to a service-account JSON with client_email.');
    }
    return {
        uploadUrl,
