...
import { appendChatMessages, getNegotiationState, listChatMessages, listChatSessions, saveNegotiationState } from '../services/chatsRepo.js';
...
export async function handleAnalysisChatPost(req, user, analysisId) {
    if (req.method !== 'POST') {
    ...
    const prompt = buildDocumentChatPrompt({
        ...
        documentType: typeof analysis.contractType === 'string'
    ...
    const result = await generateGeminiJson(prompt, chatResponseSchema(), 2048);
...
export async function handleAnalysisChatsGet(req, user, analysisId) {
    if (req.method !== 'GET') {
...
export async function handleAnalysisChatSessionsGet(req, user, analysisId) {
    if (req.method !== 'GET') {
        throw new HttpError(405, 'METHOD_NOT_ALLOWED', 'Use GET for this endpoint.');
    }
    await getOwnedAnalysis(user.uid, analysisId);
...
export async function handleNegotiationSuggestionsPost(req, user, analysisId) {
    if (req.method !== 'POST') {
        throw new HttpError(405, 'METHOD_NOT_ALLOWED', 'Use POST for this endpoint.');
    }
    const body = assertJsonObject(req.body);
    ...
    const analysis = await getOwnedAnalysis(user.uid, analysisId);
    const extractedText = await loadAnalysisDocumentText(user.uid, analysis);
    const summary = analysis.summary && typeof analysis.summary === 'object' && !Array.isArray(analysis.summary)
        ? analysis.summary
        : {};
...
export async function handleNegotiationChatPost(req, user, analysisId) {
    if (req.method !== 'POST') {
        throw new HttpError(405, 'METHOD_NOT_ALLOWED', 'Use POST for this endpoint.');
    }
    const body = assertJsonObject(req.body);
...
export async function handleNegotiationStateGet(req, user, analysisId) {
    if (req.method !== 'GET') {
        throw new HttpError(405, 'METHOD_NOT_ALLOWED', 'Use GET for this endpoint.');
    }
    await getOwnedAnalysis(user.uid, analysisId);
