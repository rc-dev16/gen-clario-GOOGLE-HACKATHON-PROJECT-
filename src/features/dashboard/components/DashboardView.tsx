...
export interface DashboardViewProps {
  ...
  searchTerm: string;
  filterStatus: 'all' | 'high-risk' | 'medium-risk' | 'low-risk';
  onSearchTermChange: (value: string) => void;
  onFilterStatusChange: (value: 'all' | 'high-risk' | 'medium-risk' | 'low-risk') => void;
  onNewAnalysis: () => void;
...
export function DashboardView({
  ...
  const stats = {
    ...
    avgCompletionScore:
      contracts.length > 0
        ? Math.round(
            contracts.reduce((sum, c) => sum + c.completionScore, 0) / contracts.length
          )
        : 0
  };
  ...
  return (
    <div className="bg-gray-50 overflow-hidden min-h-screen">
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-gray-900/3 rounded-full blur-3xl animate-float" />
        <div
          className="absolute top-3/4 right-1/4 w-80 h-80 bg-gray-800/3 rounded-full blur-3xl animate-float"
          style={{ animationDelay: '2s' }}
        />
        <div
          className="absolute top-1/2 left-1/2 w-64 h-64 bg-gray-700/3 rounded-full blur-3xl animate-float"
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-6">
        ...
        >
          <div className="text-center mb-6">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-3 text-shadow">
              Welcome back,{' '}
              <span className="text-gray-700">{userName?.split(' ')[0] || 'User'}</span>
            </h1>
            <p className="text-lg text-gray-600 font-medium">Your contract analysis dashboard</p>
            <div className="flex justify-center mt-4">
              <div className="bg-green-50 text-green-700 px-4 py-2 rounded-full text-xs font-semibold border border-green-200">
                Docs left: {docsLeft} / {maxContracts}
              </div>
            </div>
          </div>
        </div>
              ...
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-gray-600 font-medium text-sm">
                    Overview of your contract risk levels
                  </p>
                </div>
                    ...
                    style={{
                      height: `${
                        stats.totalContracts > 0
                          ? Math.max((stats.highRisk / stats.totalContracts) * 150, 15)
                          : 15
                      }px`
                    }}
                  />
                ...
                <div className="flex flex-col items-center">
                  <div
                    ...
                    style={{
                      height: `${
                        stats.totalContracts > 0
                          ? Math.max((stats.mediumRisk / stats.totalContracts) * 150, 15)
                          : 15
                      }px`
                    }}
                  />
                ...
                <div className="flex flex-col items-center">
                  <div
                    ...
                    style={{
                      height: `${
                        stats.totalContracts > 0
                          ? Math.max((stats.lowRisk / stats.totalContracts) * 150, 15)
                          : 15
                      }px`
                    }}
                  />
              ...
              <div className="flex flex-col h-full justify-center">
                <div className="text-center mb-6">
                  <h3 className="text-2xl font-bold text-gray-900 mb-3">
                    Ready to analyze more contracts?
                  </h3>
                  <p className="text-gray-600 font-medium text-base">
                    You have{' '}
                    <span className="font-bold text-gray-900 text-lg">{docsLeft}</span> free
                    analyses remaining
                  </p>
                </div>
        ...
        <div
          className={`mb-0 transition-all duration-1000 delay-900 ${
            isVisible ? 'animate-slide-up' : 'opacity-0'
          }`}
        >
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  ...
                  <div className="relative">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        onFilterStatusChange(
                          e.target.value as 'all' | 'high-risk' | 'medium-risk' | 'low-risk'
                        )
                      }
                    ...
                    >
                      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                        ...
                        <div className="flex items-center gap-4">
                          <span
                            className={`inline-flex items-center px-4 py-2 rounded-full text-sm font-bold border ${getRiskBadgeColor(
                              contract.riskLevel
                            )}`}
                          >
